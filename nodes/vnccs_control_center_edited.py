...
def _load_clips(clip_entries, selected_names):
    ...
    for name in selected_names:
        ...
        if not str(full_path).lower().endswith(".gguf"):
            gguf_only = False
    ...
    # All selected clips are GGUF → route through ComfyUI-GGUF's CLIPLoaderGGUF,
    # which parses gguf tensors and applies GGML custom-ops to the patcher.
    if gguf_only:
        loader_cls = _get_node_class_mappings().get("CLIPLoaderGGUF")
        if loader_cls is None:
            raise RuntimeError(
                "[VNCCS Control Center] GGUF text encoder selected but ComfyUI-GGUF is not installed. "
                "Install github.com/city96/ComfyUI-GGUF."
            )
        loader = loader_cls()
        clip_name = os.path.basename(paths[0])
        (clip,) = loader.load_clip(clip_name=clip_name, type=clip_type_str)
        return clip
...
def _build_control_center_pipe(repo_id, node_state, custom_model=None, custom_clip=None, custom_vae=None):
    ...
    if selected_type == "custom":
        ...
        # Honor the explicit per-type clip selection from the widget
        # (state.selected_models.clip). Fallbacks: first same-kind clip.
        ...
        selected_clip_name = ""
        if isinstance(state.get("selected_models"), dict):
    ...
    lora_states = list(loras or [])
    lora_names_present = {item.get("name", "").strip().lower() for item in lora_states if item.get("name")}
    for entry in (config.get("lora") or []):
        entry_name = (entry.get("name") or "").strip()
        if not entry_name or entry_name.lower() in lora_names_present:
            continue
        normalized = entry_name.lower()
        if normalized not in _PIPELINE_LOCAL_LORAS and not any(t in normalized for t in _PIPELINE_LOCAL_LORAS):
            continue
        _, on_disk = _find_model_on_disk(entry.get("local_path", ""))
        if not on_disk:
            continue
        print(f"[VNCCS Control Center] Auto-including pipeline-local LoRA in pipe: {entry_name}")
        lora_states.append({"name": entry_name, "auto_apply": True, "strength": 1.0})
        lora_names_present.add(normalized)

    pipe.lora_states = lora_states
...
# ── Edited-clone override endpoint ─────────────────────────────────────────────
# The original /vnccs/control_center/check route always uses prefer_remote=True
# and downloads the upstream HF catalog (which lacks Q2/Q3 entries). Register a
# separate route under /vnccs_edited_cc/* so this clone serves its packaged
# control_center.json directly without touching the VNCCS module's routes.
@server.PromptServer.instance.routes.get("/vnccs_edited_cc/check")
async def cc_edited_check(request):
    repo_id = request.rel_url.query.get("repo_id", "MIUProject/VNCCS_v3.0").strip() or "MIUProject/VNCCS_v3.0"
    catalog_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "control_center.json"))
    if not os.path.exists(catalog_path):
        return web.json_response({"error": f"missing catalog: {catalog_path}"}, status=404)
    try:
    ...
    installed = get_installed_version_info()
    visible_models = [
        e for e in config.get("models", []) if e.get("type") != "nunchaku"
    ]
    available_types = list(dict.fromkeys(
        e.get("type", "") for e in visible_models if e.get("type")
    ))
    # Mirror the original handler so LoRAs display their on-disk status
    # instead of always showing "missing" / "Download".
    enriched_loras = [
        {**entry,
         "status": "installed" if _find_model_on_disk(entry["local_path"])[1] else "missing"}
        for entry in config.get("lora", [])
    ]
    return web.json_response({
        "name":           config.get("name", "VNCCS Core Models"),
        "source":         f"edited-local:{catalog_path}",
        "available_types": available_types,
        "models": _enrich_config_entries(visible_models, "models", installed),
        "clip":   _enrich_config_entries(config.get("clip", []), "clip", installed),
        "vae":    _enrich_config_entries(config.get("vae", []), "vae", installed),
...
@server.PromptServer.instance.routes.post("/vnccs_edited_cc/download")
async def cc_edited_download(request):
    """Edited-clone download endpoint.

    The original /vnccs/control_center/download endpoint always calls
    _get_cc_config with prefer_remote=True and resolves the entry against the
    upstream HF catalog (8 models, 1 clip). Catalog entries we added to the
    clone's control_center.json (Q2/Q3 GGUF models; the GGUF text encoder
    variants) are not in that upstream catalog, so the original endpoint would
    return "Entry '...' not found in 'models'" (or 'clip', 'lora', etc.).
    This endpoint resolves against the local edited catalog instead.
    """
    try:
        validate_privileged_request(request)
    except ValueError as exc:
    ...
    key = f"cc_{category}_{name}"
    # Clone-specific queue + worker
    _DOWNLOAD_STATUS_EDITED[key] = {"status": "queued", "message": "Queued..."}
    _DOWNLOAD_QUEUE_EDITED.put((repo_id or "MIUProject/VNCCS_v3.0", key, entry))
    return web.json_response({"status": "queued", "message": f"Download queued for {name}"})

@server.PromptServer.instance.routes.get("/vnccs_edited_cc/status")
async def cc_edited_status(request):
    return web.json_response(_DOWNLOAD_STATUS_EDITED)




_DOWNLOAD_QUEUE_EDITED = queue.Queue()
_DOWNLOAD_STATUS_EDITED: "dict[str, dict]" = {}
_DOWNLOAD_TIMEOUT_EDITED = (10, 60)


def _edited_download_worker_loop():
    while True:
        task = _DOWNLOAD_QUEUE_EDITED.get()
        if task is None:
            break
        repo_id, model_key, target_model = task
        download_repo_id = target_model.get("hf_repo", repo_id)
        temp_path = ""
        try:
            _DOWNLOAD_STATUS_EDITED[model_key] = {"status": "downloading", "message": "Initializing..."}
            _validate_privileged_request_safe = None  # not used here
            url = ""
            headers = {}
            if target_model.get("url"):
                url = target_model["url"]
            ...
            _validate_https_url(url)
            response = requests.get(url, headers=headers, stream=True, allow_redirects=True, timeout=_DOWNLOAD_TIMEOUT_EDITED)
            response.raise_for_status()

            expected_name = basename_agnostic(target_model.get("local_path", "") or target_model.get("hf_path", "") or "model")

            # No enforced size cap on this side - the clone's worker just downloads.
            downloaded = 0
            temp_dir = os.path.join(folder_paths.base_path, "temp")
            os.makedirs(temp_dir, exist_ok=True)
            sanitized_name = "".join(ch for ch in model_key if ch.isalnum())
            temp_path = os.path.join(temp_dir, f"vnccs_edited_{sanitized_name}.tmp")
            ...
            with open(temp_path, "wb") as handle:
                for chunk in response.iter_content(chunk_size=8192):
                    ...
                    downloaded += len(chunk)
                    mb_done = downloaded / (1024 * 1024)
                    _DOWNLOAD_STATUS_EDITED[model_key] = {
                        "status": "downloading",
                        "message": f"{mb_done:.1f} MB",
                        "progress": 0,
                    }
            ...
            _DOWNLOAD_STATUS_EDITED[model_key] = {"status": "success", "message": "Installed"}
            # Mirror into the original module's status dict so the widget
            # (which polls /vnccs/manager/status) sees the success badge.
        except Exception as exc:
            if temp_path and os.path.exists(temp_path):
                try:
                    os.remove(temp_path)
            ...
            if is_auth_error:
                ...
                message = "File not found (404)"
            _DOWNLOAD_STATUS_EDITED[model_key] = {"status": status, "message": message}
        finally:
            _DOWNLOAD_QUEUE_EDITED.task_done()


# Only start one worker even if both modules' __init__ runs (defensive).
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "VNCCS_ControlCenter_Edited": "VNCCS Control Center Edited",
}
