...
window.VNCCS_EDITED_CC_REGISTRY = window.VNCCS_EDITED_CC_REGISTRY || {};
window.VNCCS_EDITED_CC_FETCH_PROMISES = window.VNCCS_EDITED_CC_FETCH_PROMISES || {};
...
function _injectVNCCSControlCenterEditedStyles() {
...
app.registerExtension({
    name: "VNCCS.ControlCenterEdited",

    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name !== "VNCCS_ControlCenter_Edited") return;
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            this._cc_widget_edited = new VNCCSControlCenterEditedWidget(this);
        ...
        nodeType.prototype.onConfigure = function (info) {
            ...
            this._cc_widget_edited?.restoreState();
        };
        ...
        nodeType.prototype.onRemoved = function () {
            ...
            if (this._cc_widget_edited?.pollingInterval)
                clearInterval(this._cc_widget_edited.pollingInterval);
            if (this._cc_widget_edited?._onGlobalPointerUp)
                window.removeEventListener("pointerup", this._cc_widget_edited._onGlobalPointerUp);
            if (this._cc_widget_edited?._onRegistryUpdate)
                window.removeEventListener("vnccs-edited-cc-registry-updated", this._cc_widget_edited._onRegistryUpdate);
        };
    },
});
...
class VNCCSControlCenterEditedWidget {
    constructor(node) {
        this.node = node;
        this.config   = null;
        this.state    = {};
        ...
        _injectVNCCSControlCenterEditedStyles();
        this._buildUI();
        this._startPolling();
        ...
        this._onRegistryUpdate = (e) => {
            if (!this.container?.isConnected) {
                window.removeEventListener("vnccs-edited-cc-registry-updated", this._onRegistryUpdate);
                return;
            }
            const repoId = e.detail?.repo_id;
            const myRepo = this._getRepoId();
            if (repoId && repoId === myRepo && window.VNCCS_EDITED_CC_REGISTRY[repoId]) {
                this.config = window.VNCCS_EDITED_CC_REGISTRY[repoId];
                if (!this._isUserInteracting()) this._renderAll();
            }
        };
        window.addEventListener("vnccs-edited-cc-registry-updated", this._onRegistryUpdate);
    ...
    restoreState() {
        ...
        if (!this.state.selected_models) this.state.selected_models = {};
        if (this.state.selected_model && !this.state.selected_models.clip) {
            this.state.selected_models.clip = this.state.selected_model;
        }
    ...
    _startPolling() {
        ...
        this.pollingInterval = setInterval(async () => {
            try {
                const r = await api.fetchApi("/vnccs_edited_cc/status");
                if (!r.ok) return;
    ...
    _buildHeader() {
        ...
        this.statusText.textContent = "VNCCS Control Center Edited";
    ...
    async fetchConfig(repoId, force = false) {
        ...
        const cacheKey = `vnccs_edited_cc_cache_${repoId}`;

        if (force) {
            delete window.VNCCS_EDITED_CC_REGISTRY[repoId];
            localStorage.removeItem(cacheKey);
        }

        // Show cached data immediately while fetching fresh
        if (!force && window.VNCCS_EDITED_CC_REGISTRY[repoId]) {
            this.config = window.VNCCS_EDITED_CC_REGISTRY[repoId];
            this.statusText.textContent = this.config.name || "Control Center Edited";
        ...
        if (!force && window.VNCCS_EDITED_CC_FETCH_PROMISES[repoId]) {
            try {
                const data = await window.VNCCS_EDITED_CC_FETCH_PROMISES[repoId];
                this.config = data;
                this.statusText.textContent = data.name || "Control Center Edited";
                if (!this.state.output_slot_names) this.state.output_slot_names = [];
                this._renderAll();
                this._dispatchLoraOptions();
        ...
        const fetchPromise = (async () => {
            try {
                const url = `/vnccs_edited_cc/check?repo_id=${encodeURIComponent(repoId)}${force ? "&force_refresh=true" : ""}`;
                const r = await api.fetchApi(url);
                const data = await r.json();
                if (data.error) throw new Error(data.error);
                window.VNCCS_EDITED_CC_REGISTRY[repoId] = data;
                return data;
            } finally {
                delete window.VNCCS_EDITED_CC_FETCH_PROMISES[repoId];
            }
        })();

        window.VNCCS_EDITED_CC_FETCH_PROMISES[repoId] = fetchPromise;
        ...
        try {
            ...
            this._renderAll();
            this._dispatchLoraOptions();
            window.dispatchEvent(new CustomEvent("vnccs-edited-cc-registry-updated", {
    ...
    _renderClipVaeCard(entry) {
        ...
        if (isClip) return this._buildClipVariantCard(entry);
        return this._renderClipVaeCardLegacy(entry);
    }

    _renderClipVaeCardLegacy(entry) {
        const isClip = entry._cat === "clip";
        ...
        if (this._isDownloadableStatus(status)) {
            const footer = document.createElement("div");
            footer.className = "vnccs-cc-model-card-footer";
            footer.appendChild(this._btn(this._downloadLabel(status), () => this._downloadEntry("vae", entry)));
    ...
    _buildClipVariantCard(entry) {
        const variants = (this.config.clip || [])
            .filter(e => this._sameKind(e, this._selectedKind()) && e !== entry);
        // Build the full ordered list for the dropdown (current entry first by name match).
        const sameKind = (this.config.clip || []).filter(e => this._sameKind(e, this._selectedKind()));
        const selNames = (this.state.selected_models && this.state.selected_models.clip) || "";
        const cur = sameKind.find(v => v.name === selNames)
                 || sameKind.find(v => v.name === entry.name)
                 || sameKind[0];
        if (!cur) return document.createElement("div");

        const key    = `cc_clip_${cur.name}`;
        const dls    = this.dlStatus[key] ?? {};
        const status = this._resolveStatus(dls.status, cur.status);
        ...
        if (sameKind.length > 1) {
            const names = sameKind.map(v => v.name);
            const icon = s => ({ installed:"●", missing:"↓", outdated:"↑", queued:"…", downloading:"⬇", error:"✕", auth_required:"⚠" }[s] ?? "?");
            const sel = document.createElement("select");
            sel.className = "vnccs-cc-model-card-select";
            sameKind.forEach(v => {
                const vDls = this.dlStatus[`cc_clip_${v.name}`] ?? {};
                const vSt  = this._resolveStatus(vDls.status, v.status);
                const op = document.createElement("option");
                op.value = v.name;
                op.textContent = v.name + "  " + icon(vSt);
                if (v.name === cur.name) op.selected = true;
                sel.appendChild(op);
            });
            sel.onchange = () => {
                const chosen = sameKind.find(v => v.name === sel.value);
                if (!chosen) return;
                this.state.selected_models = this.state.selected_models || {};
                this.state.selected_models.clip = chosen.name;
                this._saveState();
                this._renderAll();
            };
            footer.appendChild(sel);
        ...
        if (this._isDownloadableStatus(status)) {
            footer.appendChild(this._btn(this._downloadLabel(status), () => this._downloadEntry("clip", cur)));
        }
        if (status === "auth_required") {
            const b = this._btn("⚠ Enter Key", () => this.showApiKeyDialog("clip", cur));
            b.style.color = "#ffaa00"; b.style.borderColor = "rgba(255,170,0,0.4)";
            footer.appendChild(b);
        }
    ...
    async _downloadEntry(cat, entry) {
        ...
        this._renderAll();
        try {
            const r = await api.fetchApi("/vnccs_edited_cc/download", {
                method: "POST",
                headers: { "Content-Type": "application/json", "X-VNCCS-CSRF": "1" },
